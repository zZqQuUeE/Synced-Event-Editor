var inputs = function() {
	...
	if X_obj_input.input_select and X_obj_input.input_r {
		...
		if bozo {
			...
			if tc >= 0 {
				...
				X_obj_gui_chain.target_chain = tc
			}
		}
	...
	// move cursor
	if can_cursor_move {
		if X_obj_input.input_l and global.dpad_timers[0] < 0 and cursor_x > 200 {
			cursor_x -= 100
			global.dpad_timers[0] = 5
		}
		if X_obj_input.input_r and global.dpad_timers[1] < 0 and cursor_x < array_length(global.json.channels) * 100 {
			cursor_x += 100
			global.dpad_timers[1] = 5
		}
		if X_obj_input.input_d and global.dpad_timers[3] < 0 and cursor_y < chain_dist * 999 {
			cursor_y += chain_dist
			global.dpad_timers[3] = 5
		}
...
visible = X_obj_game.page == 0

if visible {
	inputs()
	if cursor_y > camera_get_view_height(view_camera[0]) * 1.7 {
		gui_yoffset = cursor_y - camera_get_view_height(view_camera[0]) * 1.7
	} else {
		gui_yoffset = 0
	}
}