...
if mouse_wheel_down() and device_mouse_y_to_gui(0) >= 200 {
	if shift {
		h_scroll -= 50
	} else {
		scroll -= 30
...
if mouse_wheel_up() and device_mouse_y_to_gui(0) >= 200 {
	if shift {
		h_scroll += 50
	} else {
		scroll += 30
...
if playing {
	...
	var pattern_index = (playpos * (global.json.bpm / 60)) * 4//* global.json.spd
